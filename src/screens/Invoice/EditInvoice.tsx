...
import {initialInputs, inputInvoice} from './AddInvoice';
...
function EditInvoice({navigation, route}: EditInvoiceProp) {
  ...
  const dispatchEditInvoice = useAppDispatch();
  ...
  const addInvoiceDetail = (invoice: any) => {
    let keys = Object.keys(invoice).filter(x => fieldsKeys.indexOf(x) > -1);
    for (let key of keys) {
      if (key == 'client') {
        inputChangedHandler(key, invoice[key]._id || '');
      } else if (key == 'employee') {
        inputChangedHandler(key, invoice[key]._id || '');
      } else if (key == 'selectedService') {
        setSelectedServices(invoice[key]);
  ...
  const addValue = (addedValue: string) => {
    let addServiceDetail = subServiceData.reduce((curr, value) => {
    ...
    if (addServiceDetail) {
      setSelectedServices(prevServices => [...prevServices, addServiceDetail]);
      calculateTax();
  ...
  const editInvoice = async () => {
    try {
      ...
      await dispatchEditInvoice(
        fetchUpdateInvoice({invoiceId, payload}),
      ).unwrap();

      dispatchEditInvoice(fetchInvoice());
      navigation.goBack();
    } catch (error) {
      setShowError(true);
